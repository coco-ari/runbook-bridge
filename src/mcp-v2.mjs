...
const instructions = `这是一个插件化 Agent 运维入口。项目下的环境彼此隔离；每次开始工作必须先调用 open_environment，读取该环境运维说明、插件目录和 contextToken。桌面应用不会因为 Agent 调用而连接环境；插件未连接时，请用户在桌面应用点击“连接环境”。环境部分连接时，仅调用明确显示 connected 的插件。Server 不提供任意 Shell，只能使用固定 actionId，日志和配置只能通过 sourceId/fileId。MySQL 插件固定一个数据库且只允许结构、SELECT、EXPLAIN；Redis 只允许已登记 patternId 下的 SCAN、读取和 TTL。不得索要或传递密码、DSN、SSH 命令、主机或端口。`;
...
const methodByTool = {
  list_projects: 'listProjects', list_environments: 'listEnvironments', open_environment: 'openEnvironment',
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  try {
    ...
    if (name === 'open_environment') args.clientInstanceId = clientInstanceId;
