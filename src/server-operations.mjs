...
export class ServerOperations {
  ...
  get logSnapshotCache() { return this.logSearch.logSnapshotCache; }
  get logSearchGate() { return this.logSearch.logSearchGate; }
  ...
  searchLogs(plugin, args) {
    return this.logSearch.searchLogs(plugin, args);
  }
  ...
  findFilesWithReader(reader, options) {
    return this.discovery.find(reader, options);
  }

  findFiles(plugin, options = {}) {
    return this.withRemoteReadSession(plugin, async (reader) => {
      const result = await this.findFilesWithReader(reader, { ...options, cacheScope:[plugin.projectId,plugin.environmentId,plugin.pluginInstanceId,plugin.revision] });
      return { ...result, files:result.files.map(({fromCachedListing,...file}) => file) };
    });
  }
  ...
  async searchFiles(plugin, { path: remotePath, pattern = '*', contains, maxDepth = 6, maxFiles = 100, maxMatches = 200, maxScanBytes = 16 * 1024 * 1024 } = {}) {
    ...
    return this.withRemoteReadSession(plugin, async (reader) => {
      const found = await this.findFilesWithReader(reader, { path:remotePath, pattern, maxDepth, maxResults:fileLimit, cacheScope:[plugin.projectId,plugin.environmentId,plugin.pluginInstanceId,plugin.revision] });
      const matches = [];
      let scannedBytes = 0;
      let scannedFiles = 0;
      for (const file of found.files) {
        ...
        while (scannedBytes < scanLimit && matches.length < matchLimit) {
          ...
          for (let index = 0; index < lines.length && matches.length < matchLimit; index += 1) {
            if (index % 1024 === 0) await yieldRead();
