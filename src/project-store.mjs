...
const MAC_MCP_INSTALL = "\n## macOS 的 Codex MCP 安装\n\n将应用复制到“应用程序”目录后，在终端执行；自定义安装目录请替换以下路径：\n\n```sh\ncodex mcp add --env ELECTRON_RUN_AS_NODE=1 agent-ops -- \"/Applications/Agent运维工作台.app/Contents/MacOS/Agent运维工作台\" \"/Applications/Agent运维工作台.app/Contents/Resources/app.asar/src/mcp-v2.mjs\"\ncodex mcp get agent-ops\n```\n\n桌面工作台需要保持运行并由用户主动连接环境。macOS 数据目录为 ~/.ai-ops-tool；如果使用 AI_OPS_DATA_DIR，需要同时向桌面与 MCP 传入同一个绝对路径。跨系统迁移配置后，请重新录入凭据并选择本机私钥路径。\n";

const DEFAULT_README = (name) => `# ${name}\n\n## 服务器与项目目录\n\n在这里填写项目涉及的服务器目录。\n\n## 产物清单\n\n| 本地文件 | 上传目标 | 说明 |\n| --- | --- | --- |\n| \`D:\\\\work\\\\project\\\\target\\\\app.jar\` | \`/home/deploy/app/app.jar\` | 示例，请修改 |\n\n## 部署流程\n\n1. 检查当前进程。\n2. 备份现有产物。\n3. 上传新产物。\n4. 执行项目启动命令。\n5. 检查日志确认启动成功。\n6. 失败时恢复备份。\n\n## 日志位置\n\n在这里填写日志文件完整路径、日志格式、启动成功标志和常用关联字段。Codex 会从本文档取得明确路径，并可使用结构化日志搜索。\n\n## 操作要求\n\n- 不使用 sudo。\n- 删除或覆盖文件前先备份。\n- 遇到不确定情况先询问。\n\n## Codex MCP 安装\n\n安装“Agent运维工作台”后，在 PowerShell 中执行以下命令。若修改过安装目录，请把命令中的路径替换为实际安装位置。\n\n\`\`\`powershell\ncodex mcp add --env ELECTRON_RUN_AS_NODE=1 agent-ops -- \"$env:LOCALAPPDATA\\Programs\\Agent运维工作台\\Agent运维工作台.exe\" \"$env:LOCALAPPDATA\\Programs\\Agent运维工作台\\resources\\app.asar\\src\\mcp-v2.mjs\"\n\`\`\`\n\n验证 MCP 是否注册成功：\n\n\`\`\`powershell\ncodex mcp get agent-ops\n\`\`\`\n\n注册或升级后请完全退出并重新启动 Codex。服务器连接仍需由用户在桌面工具中主动建立，MCP 不会自行登录服务器。\n${MAC_MCP_INSTALL}`;
