...
test('production renderer exposes only the structured V2 operations surface', async () => {
  ...
  assert.match(renderer, /配置已保存；可以先测试基础连接，再补充业务范围/);
  assert.match(renderer, /function pluginDiagnosticConfigurationIssue/);
  assert.match(renderer, /测试基础连接/);
  ...
  assert.match(styles, /\.connection-check-section:has\(\.diagnostic-overview\.pending\)/);
  assert.match(styles, /\.plugin-card:has\(\.plugin-form-diagnostic \.diagnostic-overview\.pending\)/);
