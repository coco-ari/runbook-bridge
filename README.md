...
当前代码包版本：`1.0.46`（版本唯一来源为 `package.json`；自动化测试会校验此处保持同步。）
...
左侧项目栏底部、“新增项目”上方提供主题菜单，可选择“浅色”“深色”或“跟随系统”。默认跟随系统；手动选择会立即应用到界面和提示消息，保存为本机偏好，重启后保持。手动模式不受系统主题变化影响，选回“跟随系统”后恢复实时跟随。收起项目栏时入口仍可见，支持键盘操作。
