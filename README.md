...
连接 Server 后，在插件详情的操作区点击「打开工作区」，即可使用左侧目录树、右侧连续 SSH 终端和底部上传任务。已有工作区时点击「继续工作区」。目录软链接可按文件夹展开，文件软链接可只读预览；链接旁显示实际目标，循环和失效链接会明确标记。目录先显示基本内容，再后台补齐软链接目标；大目录采用有界并行读取，分页复用目录快照。已读目录在工作区内缓存，点击面包屑定位并高亮对应目录，保留同级和展开状态；打开文件实时读取，路径失效时局部刷新。上传前确认实际目标目录和覆盖冲突，初版支持单文件最大 500 MiB、最多 2 个并行传输。
