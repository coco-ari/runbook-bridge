...
终端支持新增、切换和关闭独立标签；文件预览支持同时打开多个文件并保留滚动位置。终端颜色使用标准 ANSI 输出，新建终端默认自动设置 ls 分类配色和 ll 别名（常见 POSIX Shell，自动识别 GNU/BSD ls）。「目录配色」可关闭新终端自动配置，也可手动填入当前会话设置后按 Enter 生效。切换标签、返回工作区不会重复配置，不修改服务器配置文件。
