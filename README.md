...
MySQL 表格左侧竖栏提供「新增行」「复制行」「删除行」，新增与复制直接进入表格草稿，绿色表示新增、蓝色表示复制、黄色表示修改、红色表示删除；所有更改统一保存或取消，完整字段可从状态栏按钮打开侧栏编辑。Redis 左侧提供「新增 Key」，详情提供「编辑值」「删除 Key」；草稿仅保存在会话内，断线后需重新核对。保存和删除使用原位转圈及固定底栏反馈，等待超过 5 秒显示耗时，提示不会挤动表格或编辑区。写入成功与刷新失败分开显示；提交结果不确定时禁止重复保存，Redis 可检查状态或只读核实当前数据，暂时关闭删除提示仍保留核实入口。桌面人工写入不会增加 Agent/MCP 权限，详见 [MySQL 数据编辑](docs/mysql-data-editing.md) 和 [Redis 工作区](docs/redis-workspace-design.md)。
