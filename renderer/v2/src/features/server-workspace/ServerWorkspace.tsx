...
export function ServerWorkspace({ api, entry, visible, onBack, onClose }: ServerWorkspaceProps) {
  ...
  useEffect(() => {
    ...
    void poll()
    // 新任务或工作区可见性变化立即重查，避免沿用空闲时的长等待；旧查询结果仍由版本号拦截。
    return () => { disposed = true; window.clearTimeout(timer) }
  }, [api, scope, hasActiveTransfers, visible])
  ...
  return <div className="server-workspace" hidden={!visible} data-testid="server-workspace" data-workspace-key={serverWorkspaceKey(scope)}>
    ...
    {uploadPollError ? <WorkspaceNotice className="server-workspace-error" variant="destructive" data-testid="upload-poll-error">{uploadPollError}<Button size="icon-sm" variant="ghost" aria-label="收起传输状态提示" onClick={() => setUploadPollError("")}><X /></Button></WorkspaceNotice> : null}
    {dockerTabError ? <WorkspaceNotice className="server-workspace-error" variant="destructive">{dockerTabError}<Button size="icon-sm" variant="ghost" aria-label="关闭容器标签提示" onClick={() => setDockerTabError("")}><X /></Button></WorkspaceNotice> : null}
