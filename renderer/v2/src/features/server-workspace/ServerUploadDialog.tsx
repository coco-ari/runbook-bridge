...
export function ServerUploadDialog({ preparation, serverName, environmentName, identity, busy, confirming, removing, connected, needsReview, overwrite, error, onOverwrite, onRevise, onConfirm, onCancel, onReselect }: Props) {
  ...
  return <DialogContent className="server-upload-dialog sm:max-w-[780px]" showCloseButton={!busy} onInteractOutside={(event) => { if (busy) event.preventDefault() }} onEscapeKeyDown={(event) => { if (busy) event.preventDefault() }}>
    ...
    <div className="server-upload-body">
      {conflicts.length && !preparation.resume ? <div className="server-upload-batch-actions"><span>{conflicts.length} 个同名目标</span><label>应用于本批次冲突文件 <select aria-label="批量处理同名文件" value="" disabled={busy || checking || !connected} onChange={event => changeAction(conflicts.map(file => file.name), event.target.value as ServerUploadConflictAction)}><option value="" disabled>选择处理方式…</option><option value="skip">全部跳过</option><option value="overwrite">全部覆盖</option><option value="keep-both">全部保留两份</option></select></label></div> : null}
      <ul className="server-upload-confirm-files" aria-label="待上传文件" aria-busy={busy || checking}>
        {files.map((file) => <li key={file.remotePath} className="server-upload-confirm-file" data-testid="upload-file-row">
          ...
          {file.exists && !preparation.resume ? <div className="server-upload-file-conflict">
            ...
            <label>处理方式 <select aria-label={`处理同名文件 ${file.name}`} value={file.action === "skip" || file.action === "keep-both" ? file.action : "overwrite"} disabled={busy || checking || !connected} onChange={event => changeAction([file.name], event.target.value as ServerUploadConflictAction)}><option value="overwrite" disabled={Boolean(file.remote && file.remote.type !== "file")}>覆盖（需要确认）</option><option value="skip">跳过</option><option value="keep-both">保留两份</option></select></label>
          </div> : null}
      ...
      {existing > 0 ? <label className="server-upload-conflict"><WarningCircle size={17} aria-hidden="true" /><input type="checkbox" checked={overwrite} disabled={busy || checking || needsReview || !ready} onChange={(event) => onOverwrite(event.target.checked)} /><span>确认覆盖本次选择的 {existing} 个同名文件</span></label> : null}
