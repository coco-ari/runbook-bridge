...
export function MysqlTableBrowser({ api, scope, table, description, maxRows, visible, dragScope }: {
  ...
  const apply = () => protect(() => { void readPage(true, { where: where.trim(), sort }) })
  return <div className="mysql-table-browser" hidden={!visible}>
    ...
    {result ? <MysqlEditableResults api={api} scope={scope} documentKey={documentKey} sql={editSql} visible={visible} onReload={() => void readPage(true)}><MysqlQueryResults snapshot={snapshot} columnDragScope={{ workspace: dragScope, table }} kind="preview" result={result} sort={sort} onSort={next => protect(() => { setSort(next); void readPage(true, { where: where.trim(), sort: next }) })} stream={{ key: `${table}/${generation}`, loading, hasMore: hasMore && !error, onLoadMore: () => void readPage(false), message }} /></MysqlEditableResults> : <div className="mysql-browser-empty" role="status">{loading ? "正在读取数据…" : message || `输入筛选条件或直接执行，每次读取 ${pageSize} 行。`}</div>}
  </div>
}
