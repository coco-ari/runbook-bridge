...
export async function installMysqlEditUiFixture({ipcMain,registeredChannels,plugin,moduleRoot}) {
  ...
  const services={isWorkspaceRenderer:()=>true,v2Service:{mysqlEditor:editor,invokeDesktopMysqlEdit:async(owner,payload,operation,assertOwner)=>{
  ...
  return {
    ...
    close:async()=>{editor.closeOwner('fixture-read');if(live)await live.close();},
  };
}
