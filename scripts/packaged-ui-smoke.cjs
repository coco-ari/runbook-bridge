...
async function main() {
  ...
  try {
    ...
    assert.equal(inspection.apiCount, 86);
    ...
    assert.deepEqual(restartedWorkspace, {ok: true, projectCount: 0, apiCount: 86});
    assert.deepEqual(running.httpRequests, []);
    ...
    process.stdout.write(
      `Packaged React UI smoke passed (86 preload APIs, empty isolated workspace, 128px rail, restart persistence, no external requests): ${executable}\n`,
    );
  } catch (error) {
    if (running) {
