...
工作台左侧将本地项目和云项目合并在同一个列表，不显示仓库分组标题或名称左侧的云图标，已下载项目沿用统一的拖动排序。每行右侧只保留一个状态位：平时显示连接状态，云端有更新时显示主题色下载标记，本地有修改时显示警告色编辑标记，检测失败时显示错误标记，尚未下载时显示灰色下载标记。连接中、部分可用、阻塞、错误和隔离状态优先显示；悬停或键盘聚焦项目可查看连接状态、所属仓库和云同步状态。项目详情的更新、上传按钮旁显示云仓库名称或“本地项目”。关联仓库后自动列出其中的项目；首次打开云项目时下载配置，不需要导入向导，也不会自动连接服务器。
