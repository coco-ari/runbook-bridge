...
项目详情提供“更新”“上传”。配置 → 云配置沿用工作台的主题、按钮和卡片样式，仓库中的每个项目以紧凑卡片展示，提供显示开关，底部同一排显示同步状态、单个更新和单个上传，不重复显示仓库名称。工具栏提供搜索、全部显示、全部隐藏、检测更新和更新整个仓库，没有整库上传。隐藏仅影响本机工作台列表，保留已下载配置；全部显示/隐藏作用于当前仓库的所有项目，不受搜索筛选影响。
