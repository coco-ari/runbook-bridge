...
页面顶部展示仓库解锁状态和地址，“项目同步”“仓库设置”“本地备份”为独立的页内栏目。同步工具栏和底部操作栏分别占据固定空间，只有中间的项目列表滚动，操作栏不会覆盖项目。选择上传或下载后，可搜索项目、仅看已选或清空选择；全选只作用于当前筛选结果，已选但隐藏的项目仍计入总数。切换仓库设置再返回时保留勾选。预览单独展示变更及冲突选择，返回可继续调整项目；本地备份预览可返回原恢复入口。项目读取中、读取失败和空仓库分别显示，避免把加载失败误认为没有项目。
