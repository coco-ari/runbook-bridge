...
下载前检查活动编辑和文件传输；存在活动任务时先完成或取消。覆盖会断开该项目的连接，取消该项目的 Docker 读取并清除分页快照，同时使旧 Agent 上下文、审批失效；其他项目的 Docker 会话保持可用。导入以项目为原子单位，多个项目逐项显示完成或失败。每次覆盖前将本机配置和凭据保存为系统加密备份；“本地加密备份”可预览恢复。未完成事务在下次启动时恢复；无法解密或恢复的项目保持隔离，不允许继续连接。
