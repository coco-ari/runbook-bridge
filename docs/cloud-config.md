...
工作台左侧将本地项目和云项目合并在同一个列表，不显示仓库分组标题，已下载项目沿用统一的拖动排序。云项目名称旁只显示云图标：默认色表示已同步或尚未下载，主题色表示云端有更新，警告色表示本地有修改，错误色表示检测失败。关联仓库后自动列出其中的项目；首次打开云项目时下载配置，不需要导入向导，也不会自动连接服务器。
