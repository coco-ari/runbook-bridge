...
页面顶部展示仓库解锁状态和地址，完整链接及解除绑定收在“仓库设置”中。选择上传或下载后，可搜索项目；搜索只筛选展示，全选只作用于匹配结果，已选但隐藏的项目仍计入总数。列表随页面滚动，底部保留选择数量和预览入口。预览单独展示变更及冲突选择，返回可继续调整项目；本地备份预览可返回原恢复入口。项目读取中、读取失败和空仓库分别显示，避免把加载失败误认为没有项目。
