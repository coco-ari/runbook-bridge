...
支持 Apple Silicon（arm64）及 Intel（x64），分别产出 DMG 和 ZIP；系统最低版本以当前锁定 Electron 运行时和实际验证结果为准，不凭构建成功宣称旧系统可用。本方案早期实施主机为 Windows；平台验收必须绑定具体提交、架构和运行记录。Mac 真机/Runner 结果需独立取得，不能用注入 `darwin` 的单元测试替代。
