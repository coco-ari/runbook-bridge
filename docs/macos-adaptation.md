...
- Windows：语法和类型检查通过；完整单元回归 804 项通过、3 项 Mac/Unix 原生测试按平台跳过。首轮六组 UI、安装包构建、包内容校验、35 个 MCP 工具和包内 UI/凭据/模拟协议回归均通过；新增焦点交接修复后正在重新运行完整 UI。
- 已发现并修正主线已有的服务器虚拟列表 smoke 定位问题：先滚动到明确的失效链接，再验证点击不会遍历链接；没有修改产品的文件访问策略。
- Mac ARM64 和 Intel Runner 已实际通过代码检查、单元测试、Unix socket 权限/长路径/重启和 `/sbin/route` 内核查询。已修正 Command+A 自动化编辑命令、草稿确认框延迟抢焦点及 IPv6 源地址选择；当前继续验证完整 UI 和包内流程，尚不能宣布 Mac 验收完成。
- 签名/公证、真实 VPN、Finder 启动的系统 SSH Agent、Keychain 升级和完整界面对照仍待验证。当前仓库未配置 Apple 签名和公证 secrets，正式分发流程会明确失败，不能把开发包当作正式发布包。
