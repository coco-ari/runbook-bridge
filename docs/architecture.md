...
- 桌面 MySQL 查询通过显式 preload/IPC 入口进入 `src/v2-service.mjs`，复用内置操作门禁、MySQL runtime 和审计；仅允许表列表、表结构、每次最多 100 行预览与单条只读查询。表预览可传入有界的 WHERE 表达式、字段排序和分页偏移；后端只序列化通过既有只读策略的表达式，拒绝子查询与逃逸到其他 SQL 子句的输入。点击表时按需读取结构和首批 20 行，最多保留 6 个表标签，每表最多 1000 行或 4 MB。隐藏表只保留会话状态、不挂载结果 DOM；关闭表及工作区时清理状态，迟到响应不得回填。SQL 基础语法诊断按需加载已安装解析器的 MySQL 构建，仅在本地运行；用户点击复制时，浏览器剪贴板被桌面权限策略拒绝会回退到原生复制命令，保持全局权限拒绝策略不变，不增加剪贴板读取 API；字段补全使用会话内的有界表结构缓存。桌面操作记录为 `actor: user`，Agent 的环境 contextToken 验证保持不变。数据库读取持有环境操作协调锁，连接配置修改期间拒绝进入，避免旧配置请求落到新连接。
